...
IMPORTANT: the NumPy fallback classes are defined unconditionally (under
_Numpy*-prefixed names), never only inside the "sklearn unavailable" branch.
A model trained with the fallback on one machine (e.g. no scikit-learn) and
then unpickled on another machine (e.g. Streamlit Cloud, where scikit-learn
IS installed) must still resolve to these exact classes -- if the fallback
classes were named plainly "StandardScaler"/"LogisticRegression" and only
existed when sklearn was absent, pickle would instead resolve those names to
the *real* scikit-learn classes wherever scikit-learn is present, silently
reconstructing a broken half-initialized sklearn estimator (missing
attributes like classes_) instead of the fallback object that was actually
trained. Keeping the fallback classes under distinct, always-defined names
...
def _numpy_train_test_split(X, y, test_size=0.2, random_state=None, stratify=None):
...
def _numpy_accuracy_score(y_true, y_pred):
...
def _numpy_classification_report(y_true, y_pred, output_dict=False):
    cm = _numpy_confusion_matrix(y_true, y_pred)
...
class _NumpyDecisionTreeClassifier:
...
class _NumpyRandomForestClassifier:
    ...
    def fit(self, X, y):
        ...
        for _ in range(self.n_estimators):
            ...
            tree = _NumpyDecisionTreeClassifier(
...
try:
    ...
    LogisticRegression = _NumpyLogisticRegression
    DecisionTreeClassifier = _NumpyDecisionTreeClassifier
    RandomForestClassifier = _NumpyRandomForestClassifier
    train_test_split = _numpy_train_test_split
    accuracy_score = _numpy_accuracy_score
    confusion_matrix = _numpy_confusion_matrix
    classification_report = _numpy_classification_report
